...
This module implements an intelligent self-review processing system that
- uses MCP tools for copy-editing, summarization, and word cloud generation,
- includes langgraph tools for achievement extraction and review scorecard generation,
- returns a review complete flag which will be false if the list of achievements has less than 3 items.
"""
...
async def after_achievements_parser(state: GraphState) -> dict:
    ...
    summary = AIMessage(
        content=(
            f"Received {len(achievements.items)} achievements from tool.\n"
        )
    )    
...
def should_continue_review(state: GraphState) -> Literal["no_continue", "continue"]:
...
async def build_graph() -> StateGraph:
    ...
    builder.add_conditional_edges("after_achievements_parser", should_continue_review, {
