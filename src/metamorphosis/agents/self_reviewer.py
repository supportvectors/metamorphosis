...
def _find_tool(tool_name: str):
    ...
    if not tool:
        raise_mcp_tool_error(f"tool_name tool not found", tool_name="tool_name", operation="tool_lookup")
    return tool
...
async def initialize_components() -> None:
    ...
    if not tools or not isinstance(tools, list):
        raise_postcondition_error("No tools retrieved from MCP server", context={"tools_count": len(tools) if tools else 0}, operation="mcp_tool_initialization")
...
async def copy_editor_node(
    ...
    if not copy_edited_text or not isinstance(copy_edited_text, str):
        raise_postcondition_error("Copy edit output validation failed", context={"has_text": bool(copy_edited_text), "text_type": type(copy_edited_text).__name__}, operation="copy_edit_validation")
...
async def summarizer_node(
    ...
    if not summary or not isinstance(summary, str):
        raise_postcondition_error("Summarizer output validation failed", context={"has_text": bool(summary), "text_type": type(summary).__name__}, operation="summarizer_validation")
...
async def wordcloud_node(
    ...
    if not wordcloud_path or not isinstance(wordcloud_path, str):
        raise_postcondition_error("Word cloud output validation failed", context={"has_path": bool(wordcloud_path), "path_type": type(wordcloud_path).__name__}, operation="wordcloud_validation")
...
async def run_graph(
    ...
    try:
        ...
        if not isinstance(result, dict) or "original_text" not in result:
            raise_postcondition_error("Graph execution result validation failed", context={"result_type": type(result).__name__, "has_original_text": "original_text" in result if isinstance(result, dict) else False}, operation="graph_execution_validation")
